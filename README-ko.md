...
## 스마트 자동 스케일링 (v2.3 — L1/L2/L3)

세션 복잡도에 따라 출력 깊이가 자동 조절됩니다:

| 레벨 | 토큰 예산 | 트리거 | 포함 섹션 |
|------|-----------|--------|-----------|
| **L1** | ~100 토큰 | 10개 미만 메시지 OR 1개 파일 수정 | Time, Topic, Summary, Next Step |
| **L2** | ~300 토큰 | 10-50개 메시지 OR 2-10개 파일 수정 | L1 + User Requests, Key Decisions, Failed Approaches, Files Modified |
| **L3** | ~500 토큰 | 50개+ 메시지 OR 10개+ 파일 수정 | 전체 템플릿 (모든 섹션) |

메시지 수와 파일 수가 다른 레벨을 가리키면 **높은 쪽**이 적용됩니다. 수동 레벨 선택 없이 `/handoff`만 실행하면 됩니다.
